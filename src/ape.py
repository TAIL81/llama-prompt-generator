...
from groq.types.chat.chat_completion_message_param import ChatCompletionMessageParam # チャット補完メッセージの型ヒント
...
class APE:
    ...
    async def __call__(
        ...
        candidate_dicts: List[Dict[str, str]] = [{"prompt": c} for c in candidates] # 候補プロンプトを辞書形式に変換
        customizable_variable_list: List[str] = list(demo_data.keys()) # カスタマイズ可能な変数リスト
        ...
        logging.info("Rating initial candidates generated by APE.rewrite...") # 初期候補の評価を開始
        ...
        for i in range(epoch):
            ...
            if more_candidate_prompt:
                current_rating_candidates: List[Dict[str, str]] = [
                    best_candidate_obj,
                    {"prompt": more_candidate_prompt}, # 現在の最良候補と新しく生成された候補を比較
                ]
                logging.info(
    ...
    def _call_groq_api(
        self, messages: List[ChatCompletionMessageParam], method_name: str # Groq API呼び出しの共通化
    ) -> Optional[str]:
        ...
        try:
            ...
            logging.error(f"APE.{method_name} - Unexpected error: {e}") # その他の予期せぬエラーのログ出力
            return None

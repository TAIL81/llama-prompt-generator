...
def test_app_config_missing_translations(mock_env, tmp_path):
    ...
    # 不正なパスで設定を初期化
    with patch("src.app.AppConfig.Config.env_file", new=None), \
         patch("src.app.AppConfig.TRANSLATIONS_PATH", str(invalid_path)):
        config = AppConfig()
...
def test_gradio_ui_setup():
    ...
    with patch("src.app.gr.Blocks") as mock_blocks, \
         patch("src.app.create_metaprompt_tab"), \
         patch("src.app.create_translation_tab"), \
         patch("src.app.create_evaluation_tab"), \
         patch("src.app.create_soe_tab"), \
         patch("src.app.create_calibration_tab"):
...
def test_process_cleanup():
    ...
    with patch("src.app.psutil.Process", return_value=mock_process), \
         patch("src.app.sys.exit") as mock_exit:
...
def test_main_execution():
    ...
    with patch("src.app.load_dotenv"), \
         patch("src.app.demo.launch") as mock_launch, \
         patch("sys.exit") as mock_exit, \
         patch("src.app.kill_child_processes"):

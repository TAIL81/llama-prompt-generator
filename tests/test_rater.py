...
def test_groq_config_defaults():
    ...
    config = GroqConfig()
    assert config.get_output_model == "meta-llama/llama-guard-4-12b"
    ...
    assert config.temperature_get_output == 0.0
    assert config.temperature_rater == 0.0
...
def test_rater_json_parse_fallback(rater_instance):
    ...
    candidates = [{"prompt": "候補1", "output": "出力1"}, {"prompt": "候補2", "output": "出力2"}]
...
def test_rater_api_error(rater_instance):
    """APIエラー時の挙動を検証"""
    # モック設定（APIエラー）
    rater_instance.groq_client.chat.completions.create.side_effect = APIError("Test error")
